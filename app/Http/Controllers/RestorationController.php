...
class RestorationController extends Controller
{
    ...
    public function store (Request $request, $lending_id)
    {
        try {
            ...
            if ((int)$totalStuffRestoration > (int)$lending['total_stuff']) {
            ...
            } elseif ((int)$totalStuffRestoration < (int)$lending['total_stuff']) {
                return ApiFormatter::sendResponse(400, 'bad request', 'Total barang yang dikembalikan kurang dari yang dipinjam!');
                ...
                //menghitung databaru yg akan dimasukan ke stuffstock
                //$totalAvailblestock penjumlahan sebelumny dan barang bagus yang kembali 
                $totalAvailableStock = (int)$stuffStock['total_available'] + (int)$request->total_good_stuff;
                //$totalDefecStock penjumlahan data rusak sebelumnya dan rusak yang kembali
